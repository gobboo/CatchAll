...
""" class AccessPoint:
    def __init__(self, wlan='wlp4s0', ip="192.168.45.1", netmask="255.255.255.0"):
        self.wlan = wlan
        self.ip = ip
        self.netmask = netmask

    def _start_router(self):
        self._execute_shell('systemctl stop wpa_supplicant.service')

        self._execute_shell('ip link set wlp4s0 down; ip addr flush dev wlp4s0; ip link set wlp4s0 up; ip addr add {} dev wlp4s0'.format(self.ip))
        stripped_ip = self.ip.rindex('.')
        ipparts = self.ip[0:stripped_ip]


        dns = 'dnsmasq --dhcp-authoritative --interface={} --dhcp-range={}.20,{}.100,{},4h'.format(self.wlan, ipparts, ipparts, self.netmask)
        self._execute_shell(dns)

        self._execute_shell('hostapd -B /etc/hostapd/hostapd.conf')

        return True

    def _stop_router(self):
        self._execute_shell('pkill hostapd && pkill dnsmasq')
        self._execute_shell('ip link set wlp4s0 down; ip addr flush dev wlp4s0; ip link set wlp4s0 up')
        self._execute_shell('systemctl start wpa_supplicant')
        return True
    ...
    def _execute_shell(self, command_string):
        subprocess.check_call(command_string, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
 """
...
class AccessPoint:
    ...
    def _pre_start(self):
        try:
            self._execute_shell('systemctl stop wpa_supplicant')
    ...
    def _stop_router(self):
        ...
        logging.debug(r.strip())
        self._execute_shell('nmcli radio wifi on')
        self._execute_shell('ip link set ' + self.wlan + ' down')
        self._execute_shell('ip addr flush ' + self.wlan)
        self._execute_shell('ip link set ' + self.wlan + ' up')
